...
def RT(myTuple, SplineParam, result_list, lock):
    ...
    for i in myTuple:
        ...
        if M[5] != 0:
            T = (60 - M[2]) / M[5]  # t= (40-z0)/kz 40 = 2* focus
...
def calcRayIntersectM2(Ri2, M2, show_plot:bool = True):
    dt_object = current_time()
    if show_plot:
        print("RayTracing of M2 Start Time =", dt_object)

    M2x = M2[0]
    M2y = M2[1]
    M2z = M2[2]
    SplineParam = spline_calculation.SplineCalculation_M2(M2x, M2y, M2z)
    ...
    chunks = list(RTR_MT_M1_XY_input.chunkify(Ri2, len(Ri2) // num_of_proc))
    ...
    print("RayTracing of M2 End Time:", dt_object_end)
    print("RayTracing of M2 Duration:", dt_object_end - dt_object)
    if show_plot:
        plt.show()
