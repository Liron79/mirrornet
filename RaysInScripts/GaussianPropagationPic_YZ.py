...
xdelta = 1
ydelta = 1
x = np.arange(-47, 47, 1 / fx)
# y = x
y = np.arange(0, 64, 1 / fx)
...
output_file_path = os.path.join(dir_path, f"gauss_out_thresh.csv")
with open(output_file_path, "w+", newline="") as f:
    columns = ["x", "y", "z", "", "ky", "kz", "ex", "ey", "ez", "distance", "amp", "status", "ray_index"]
        ...
        for j in range(Ny):
            ...
            for k in range(len(fx)):
                for l in range(len(fy)):
                    ...
                    Ri = [7 + i, -47 + j, 60, kx, ky, -1, 1, 0, 0, 0, amp, 0, idx]
                    idx += 1
                    csv_writer.writerow(Ri)
# Maxvalue1=max[WigF]
# Maxvalue=max([WigF])
...
with open(pulse_file_path, "w+", newline="") as f:
    ...
    idx = 1
    print(f"{X.shape=}")
    for ti, i in enumerate(np.arange(-46, 48, 1)):
        for tj, j in enumerate(np.arange(0, 65, 1)):
            t = Nx * ti + tj
